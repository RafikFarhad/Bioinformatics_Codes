...
def globalAllignment(str1, str2):
    errorMat = pd.read_csv('inputs/PAM250.txt', delim_whitespace=True)
    len1 = len(str1) + 1
    len2 = len(str2) + 1
    mat = []
    path = []
    ...
    for i in range(1, len1):
        mat.append([0] + [-100000000 for _ in range(len2-1)])
        path.append([0 for _ in range(len2)])
    ...
    return (mat[len1-1][len2-1], printRecursive(str1, str2, path, len1-1, len2-1))
